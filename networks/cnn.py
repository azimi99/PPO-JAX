...
class Actor(nn.Module):
    action_dim: int
    cnn_layers: List[Tuple[int, int, int]] = field(
    ...
    @nn.compact
    def __call__(self, x):
        for filter, kernel, stride in self.cnn_layers:
            x = nn.Conv(features=filter, kernel_size=(kernel, kernel), strides=(stride, stride))(x)
            x = self.relu(x)
...
class Critic(nn.Module):
    ...
    def __call__(self, x):
        for filter, kernel, stride in self.cnn_layers:
            x = nn.Conv(features=filter, kernel_size=(kernel, kernel), strides=(stride, stride))
            x = nn.relu(x)
        for hidden_size in self.hidden_layers:
            x = nn.Dense(hidden_size, kernel_init=layer_init(np.sqrt(2)))(x)
            x = self.activation_fn(x)
